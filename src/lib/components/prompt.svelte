...
<input class="w-0" bind:value={file} id="hidden-upload" type="file" hidden disabled={!message} accept="image/jpeg,image/jpg,image/gif,image/png,image/apng,image/tiff,video/mp4,video/webm,video/x-matroska,video/quicktime,video/x-flv,video/x-msvideo,video/x-ms-wmv,video/mpeg"/>
<form class="input-group input-group-divider grid-cols-[auto_1fr_auto] rounded-container-token">
	<div class="flex flex-col input-group-shim !p-0">
		...
		{#if file}
		<button class="btn rounded-none variant-filled-error h-full" title="Remove the attachment" on:click={() => file = ''}><Icon icon="mdi:attachment-minus" class="w-6 h-6" /></button>
		{:else}
